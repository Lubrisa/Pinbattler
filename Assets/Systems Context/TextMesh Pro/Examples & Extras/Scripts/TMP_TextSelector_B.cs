...
namespace TMPro.Examples
{
    ...
    public class TMP_TextSelector_B : MonoBehaviour, IPointerEnterHandler, IPointerExitHandler, IPointerClickHandler, IPointerUpHandler
    {
        ...
        void RestoreCachedVertexAttributes(int index)
        {
            ...
            // Restore UV0S
            // UVS0
            Vector2[] src_uv0s = new Vector2[m_cachedMeshInfoVertexData.Length];
            for (int i = 0; i < m_cachedMeshInfoVertexData.Length; i++)
            {
                src_uv0s[i] = m_cachedMeshInfoVertexData[materialIndex].uvs0[i];
            }
            Vector2[] dst_uv0s = new Vector2[m_TextMeshPro.textInfo.meshInfo.Length];
            for (int i = 0; i < m_TextMeshPro.textInfo.meshInfo.Length; i++)
            {
                dst_uv0s[i] = m_TextMeshPro.textInfo.meshInfo[materialIndex].uvs0[i];
            }
